...
public class QReki {
    ...
    static int kyureki[] = new int[4]; // 旧暦 [0]:年 [2]:月 [3]:日
    static int year, month, day;

    static String jkdata[] = { "庚(かのえ)", "辛(かのと)", "壬(みずのえ)", "癸(みずのと)",
            "甲(きのえ)", "乙(きのと)", "丙(ひのえ)", "丁(ひのと)", "戊(つちのえ)", "己(つちのと)" };
    static String jshidata[] = { "申(さる)", "酉(とり)", "戌(いぬ)", "亥(い)", "子(ね)",
            "丑(うし)", "寅(とら)", "卯(う)", "辰(たつ)", "巳(み)", "午(うま)", "未(ひつじ)" };
     ...
     * @param mm		西暦の月
     * @param dd		西暦の日
     * @return			旧暦 yyyy.mm.dd
     */
    public String Kyureki(int yyyy, int mm, int dd) {
        year = yyyy;
        month = mm;
        day = dd;
        tm = YMDT2JD(year, month, day, 0, 0, 0);
        calc_kyureki(tm);
        return kyureki[0] + "年" + (kyureki[1]==1?"閏":"")
                + kyureki[2] + "月" + kyureki[3] + "日";
    }
     ...
     * @return
     */
    public String RokuYo(int yyyy, int mm, int dd) {
        year = yyyy;
        month = mm;
        day = dd;
        tm = YMDT2JD(year, month, day, 0, 0, 0);
     ...
     * @return
     */
    public String JyukanJyunishi(int yyyy, int mm, int dd) {
        year = yyyy;
        month = mm;
        day = dd;
//		tm = YMDT2JD(year, month, day, 0, 0, 0);
//		calc_kyureki(tm);
//		return jkdata[kyureki[0] % 10] + jshidata[kyureki[0] % 12];
        return jkdata[year % 10] + jshidata[year % 12];
    }
    ...
    // =========================================================================
    static double YMDT2JD(int year, int month, int day, int hour, int min,
                          int sec) {
