...
class MarkList {
    ...
    fun getGroupList(firstTitle: String):List<String> {
        Log.d(TAG,"getGroupList: "+ mMarkList.size+" "+firstTitle)
    ...
    fun getMarkNum(sp: PointD, mapData: MapData): Int {
        for (i in 0..mMarkList.size - 1) {
            ...
            Log.d(TAG, "getMarkNum: " + i + " " + listPos.distance(sp) + listPos + " " + sp)
    ...
    fun getMarkNum(title: String, group: String): Int {
        Log.d(TAG,"getMarkNum: "+title+" "+group)
        for (i in 0..mMarkList.size - 1) {
            if (mMarkList[i].mTitle.compareTo(title) == 0) {
                if (0 == group.length)
                    return i
    ...
    fun setMarkInputDialog(c: Context, title: String, message: String, operation: Consumer<String>) {
        ...
        var markTypeAdapter = ArrayAdapter(c, R.layout.support_simple_spinner_dropdown_item, MarkData.mMarkName)
        spMarkType.adapter = markTypeAdapter
        var markSizeAdapter = ArrayAdapter(c, R.layout.support_simple_spinner_dropdown_item, MarkData.mSizeName)
...
//        var data = message.split(',')
        ...
        if (5 < data.size) {
            ...
            if (9 < data.size && (0 < data[8].length && 0 < data[9].length)) {
                Log.d(TAG,"setMarkInputDialog: "+data[8]+" "+data[9])
