...
//            ylib.fileSelectDialog(mDataDirectory, "*.gpx", true, iGpxFileSet);
    ...
    /**
     * GPXデータを取り込んでデータを更新する
     * @param filePath      GPXファイルパス
     * @param gpxData       更新前のGPXデータ
     * @return              更新後のGPXデータ
     */
    private String[] updateGpxData(String filePath, String[] gpxData) {
        ...
        gpxData[mListData.getTitlePos("年月日")] = firstData.getDate();                              //  年月日
        gpxData[mListData.getTitlePos("時間")] = firstData.getTime();                                //  時間
        gpxData[mListData.getTitlePos("緯度")] = ylib.roundStr(firstData.getLatitude(),mDigitNo);    //  緯度
        gpxData[mListData.getTitlePos("経度")] = ylib.roundStr(firstData.getLongitude(),mDigitNo);   //  経度
        gpxData[mListData.getTitlePos("高度")] = ylib.roundStr(firstData.getElevator(),mDigitNo);    //  高度
        gpxData[mListData.getTitlePos("移動距離")] = ylib.roundStr(lastData.getDisCovered(),mDigitNo);  //  移動距離(累積距離)(km)
        gpxData[mListData.getTitlePos("移動時間")] = ylib.Sec2Time(lastData.getLap());        //  移動時間
        gpxData[mListData.getTitlePos("最大高度")] = ylib.roundStr(gpsDataReader.getMaxElevator(),mDigitNo);    //  最大高度
        gpxData[mListData.getTitlePos("最小高度")] = ylib.roundStr(gpsDataReader.getMinElevator(),mDigitNo);    //  最小高度
        gpxData[mListData.getTitlePos("GPX")] = ylib.getNameWithoutExt(filePath);               //  GPXファイル名
        return gpxData;
    }

    /**
     * GPXファイルを年と分類でフォルダに分けて移動する
     * @param fname         GPXファイル名
     * @param year          対象年
     * @param category      分類名
     * @return              移動結果
     */
    private boolean moveGpxData(String fname, String year, String category) {
        String fpath = gilib.getGpxPath(fname);
        String destPath = mDataDirectory + "/" + year;
    ...
    private void init() {
        ...
        mBtGpxOperation.setEnabled(false);
