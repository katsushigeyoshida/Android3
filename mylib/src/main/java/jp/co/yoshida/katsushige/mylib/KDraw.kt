 ...
 *
 *  関数
 *      setInitScreen(width: Int, height: Int)      スクリーンの大きさ設定
 *  Surface処理
 *      initSurface(surfaceHolder: SurfaceHolder, callback: SurfaceHolder.Callback) SurfaceViewを使用するための初期化
 *      initSurfaceScreen(backColor: Int, Width: Int, Height: Int)  SurfaceView画面の初期化
 *      termSurface()                           SurfaceView での終了処理(メモリ開放)
 *      lockCanvas()                            SurfaceViewの描画の前処理
 *      unlockCanvasAndPost()                   SurfaceViewの描画の後処理
 *      setOffScreen(Width: Int, Height: Int)   SurfaceViwで使用するBitmapの初期化
 *
 *  座標系
 *      setAspectFix()                          アスペクト比固定
 *      cnvWorld2View(wp: PointD): PointD       ワールド座標からビュー(スクリーン)座標に変換
 *      cnvWorld2View(wrect: RectD): RectD      ワールド座標からビュー(スクリーン)座標に変換
 *      cnvScreen2WorldX(sl: Double): Double    X方向の長さをスクリーン座標からワールド座標に変換
 *      cnvScreen2WorldY(sl: Double): Double    Y方向の長さをスクリーン座標からワールド座標に変換
 *      cnvWorld2ScreenX(wl: Double): Double    X方向の長さをワールド座標からスクリーン座標に変換
 *      cnvWorld2ScreenY(wl: Double): Double    Y方向の長さをワールド座標からスクリーン座標に変換
 *
 *      invertedPointD(p: PointD): PointD       座標をスクリーン座標で(倒立に)変換
 *      invertedRectD(rect: RectD): RectD       座標をスクリーン座標で(倒立に)変換
 *      invertedRectD(rect: RectD): RectD       四角形をスクリーン座標で(倒立に)変換
 *
 *  描画属性
 *      backColor(color: Int)                   背景色を設定する
 *      setProperty(color: String, strokeWidth: Float, strokeStyle: Paint.Style)    描画属性の設定
 *      setProperty()                           図形描画属性の反映
 *      setTextProperty(size: Double, strokWidth: Double)   文字列の描画属性を設定(スクリーン座標)
 *      setColor(color: String)                 描画カラーの設定
 *      setStyle(style: Paint.Style)            描画スタイルを設定
 *      setTextColor(color: String)             文字色の設定
 *      setTextSize(size: Double)               文字サイズを設定する
 *      setTextProperty()                       テキスト描画属性の反映
 *
 *  World座標系
 *      setWorldTextProperty(size: Double, strokWidth: Double)  文字列の描画属性を設定(ワールド座標)
 *
 *      drawWLine(l: LineD)                     ワールド座標で線分の描画
 *      drawWLine(sp: PointD, ep: PointD)       ワールド座標で線分の描画
 *      drawWRect(rect: RectD)                  ワールド座標に四角形を描画
 *      drawWCircle(c: CircleD)                 ワールド座標で円の描画
 *      drawWCircle(cp: PointD, r: Double)      ワールド座標で円の描画
 *      drawWArc(arc: ArcD)                     ワールド座標で円弧の描画
 *      drawWArc(cp: PointD, r:Double, sa: Double, ea: Double)  ワールド座標で円弧の描画
 *      drawWPath(wplist: List<PointD>)         ワールド座標でパス(ポリゴン)の描画
 *      drawWText(text: String, p: PointD, rotate: Double = 0.0, ha: HALIGNMENT = HALIGNMENT.Left, va: VALIGNMENT = VALIGNMENT.Bottom)  ワールド座標に文字列を描画
 *      drawWTextWithBox(text: String, p: PointD, rotate: Double = 0.0, ha: HALIGNMENT = HALIGNMENT.Left, va: VALIGNMENT = VALIGNMENT.Bottom)   ワールド座標に文字列と四角形を描画
 *
 *  スクリーン座標系
 *      drawLine(sp: PointD, ep: PointD)        スクリーン座標に線分を描画
 *      drawCircle(cp: PointD, r: Double)       スクリーン座標に円の描画
 *      drawArc(cp: PointD, r: Double, sa: Double, ea: Double)  スクリーン座標に円弧の描画
 *      drawRect(rect: RectD)                   スクリーン座標に四角形を描画
 *      drawPath(plist: List<PointD>)           スクリーン座標でパス(ポリゴン)の描画
 *      drawText(text: String, p: PointD)       スクリーン座標に文字列を描画
 *      drawText(text: String, p: PointD, rotate: Double, ha: HALIGNMENT = HALIGNMENT.Left, va: VALIGNMENT= VALIGNMENT.Bottom)  スクリーン座標に文字列を描画
 *      drawTextBox(text: String, p: PointD, rotate: Double, ha: HALIGNMENT = HALIGNMENT.Left, va: VALIGNMENT= VALIGNMENT.Bottom)   文字列の背景表示
 *      drawTextWithBox(text: String, p: PointD, rotate: Double = 0.0, ha: HALIGNMENT = HALIGNMENT.Left, va: VALIGNMENT = VALIGNMENT.Bottom)    スクリーン座標に文字列と四角形を描画
 *
 *      getAligmentOffset(text: String, ha: HALIGNMENT, va: VALIGNMENT): PointD 文字列のアライメントからオフセット値を求める
 *      getStringRect(text: String, pos: PointD): RectD 文字列の領域座標の取得
 *      getTextSize(): Double                   文字サイズを取得する
 *      getDecent(): Double                     文字のベースラインよりも下の高さを取得
 *      measureText(text: String): Double       文字列の長さを取得
 *
 *  その他
