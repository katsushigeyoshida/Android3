...
//  androidでは DoubleのPointクラスがなかったので作成
/**
 ...
 * angle(): Double
 * angle(p: PointD): Double
 * fromPoler(r: Double, th:Double)
 * offset(offset: PointD)
 * offset(dx: Double, dy: Double)
 * getOffset(p:PointD): PointD
 * length(): Double
 * distance(pos: PointD): Double
 * center(pos: PointD): PointD
 * toCopy(): PointD
 * toFloat(): PointF
 * toString(): String
 * isEmpty(): Boolean
 * rotatePoint(cp: PointD, rotate: Double)
 * rotateOrg(rotate: Double)
 * scale(scale: Double)
 * scalePoint(cp: PointD, scale: Double)
 */
class PointD() {
    var x = 0.0
    var y = 0.0
