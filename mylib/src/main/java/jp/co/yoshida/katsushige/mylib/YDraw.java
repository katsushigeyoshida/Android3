...
/**
 * SurfaceViewに対応したグラフィックライブラリ
 *
 * YDraw(Canvas c)                      コンストラクタ
 * YDraw(SurfaceHolder holder)          コンストラクタ
 * setViewArea(int width, int height)   描画領域の大きさを設定、ダブルバッファのビットマップを作成
 * setWorldArea(float left, float top, float right, float bottom)   論理座標を設定する
 * lockCanvas()                         描画モードにする(表示はされない)
 * lockCanvas2()                        lockCanvas()のサブ関数座標系を設定する
 * unlockCanvasAndPost()                描画モードを解除してデータを表示する
 * setCanvas(Canvas c)                  Canvas を設定する
 * setOffScreen(int width, int height)  ダブルバッファの描画ビットマップの作成
 *
 * backColor(int color)                 背景色を設定する
 *
 * Scale(float sx, float sy)            Canvas に直接スケールを設定(未使用)
 * Scale(float sx, float sy, float px, float py)    Canvas に直接スケールを設定(未使用)
 * Translate(float dx, float dy)        Canvas に直接指定して座標を移動(未使用)
 * Rotate(float degrees)                Canvas に直接指定して回転(未使用)
 * Rotate(float degrees, float px, float py)        Canvas に直接指定して原点をして回転(未使用)
 * Skew(float sx, float sy)             Canvas を直接指定して座標を傾ける(未使用)
 *
 * setScale(float sx, float sy)         座標系のスケールを設定する
 * getScaleX()
 * getScaleY()
 * setScale(float sx, float sy, float px, float py) 座標系の4スケールを原点を指定して設定
 * setTranslate(float dx, float dy)     座標の移動
 * setRotate(float degrees)             座標を回転する
 * setRotate(float degrees, float px, float py)     原点を指定して座標を回転
 * setSkew(float sx, float sy)          座標を傾ける
 * setColor(int color)                  色の設定
 * getColor16(int colorNo)              16色の内から番号で指定した取得
 * getColor15(int colorNo)              白を除く15色から番号で指定した取得
 *
 * setStyle(Paint.Style style)          スタイル設定 塗潰し/塗潰し+周囲/周囲のみ
 * setStrokeWidth(float w)              線幅や文字の太さを設定する
 * setTextSize(float size)              文字サイズの設定
 * == 座標変換 ==
 * cnvX(float x)                        (内部関数)X方向の座標変換
 * cnvY(float y)                        (内部関数)Y方向の座標変換
 * cnvWorld2ScreenX(float wl)           X方向の長さをワールド座標からスクリーン座標に変換する
 * cnvWorld2ScreenY(float wl)           Y方向の長さをワールド座標からスクリーン座標に変換
 * cnvScreen2WorldX(float sl)           X方向の長さをスクリーン座標からワールド座標に変換
 * cnvScreen2WorldY(float sl)           Y方向の長さをスクリーン座標からワールド座標に変換
 * cnvWorld2Screen(PointF wp)           ワールド座標からスクリーン座標に変換する(点座標)
 * cnvWorld2Screen(RectF wf)            ワールド座標からスクリーン座標に変換する(四角座標)
 *
 * == ワールド座標描画
 * drawWRect(RectF rect)                ワールド座標で四角を描画
 * fillWRect(RectF rect)                ワールド座標で四角を描画する(塗潰し)
 * drawWOval(RectF rect)                ワールド座標で楕円を描画
 * fillWOval(RectF rect)                ワールド座標で楕円を描画する(塗潰し)
 * drawWCircle(PointF cp, float r)      ワールド座標で円を描画
 * fillWCircle(PointF cp, float r)      ワールド座標で円を描画する(塗潰し)
 * drawWLine(PointF ps, PointF pe)      ワールド座標でせんを描画する
 * drawWPoint(PointF wp)                ワールド座標で点を描画する
 * drawWString(String text, PointF wp)  ワールド座標で文字列を表示、起点は左上
 * drawWString(String text, PointF wp, TEXTALIGNMENT ta)    ワールド座標でアライメントを指定して文字列を表示
 * drawWStringCenter(String text, PointF wp)    ワールド座標で文字列の中心を指定して文字列を表示
 * drawWPathOnText(String text, PointF wps, PointF wpe, TEXTALIGNMENT ta)   ワールド座標で２点で指定した線に沿って文字列を表示する
 *
 * == スクリーン座標描画 ==
 * drawRect(float x, float y, float width, float height)    四角を表示する(塗潰しなし)
 * drawRect(RectF rect)                 四角を表示(塗潰しなし)
 * fillRect(float x, float y, float width, float height)    四角を表示(塗潰しあり)
 * fillRect(RectF rect)                 四角を表示(塗潰しあり)
 * fillOval(float x, float y, float width, float height)    楕円の表示(塗潰しあり)
 * fillOval(RectF rect)                 楕円表示(塗潰し)
 * drawOval(float x, float y, float width, float height)    楕円を表示(塗潰しなし)
 * drawOval(RectF rect)                 楕円表示(塗潰しなし)
 * fillCircle(float cx, float cy, float r)                  円の表示(塗潰しあり)
 * fillCircle(PointF cp, float r)       塗潰しの円の描画
 * drawCircle(float cx, float cy, float r)                  円の表示(塗潰しなし)
 * drawCircle(PointF cp, float r        円を描画
 * drawLine(float startx, float starty, float endx, float endy)     線分の表示
 * drawLine(PointF ps, PointF pe)       線分描画
 * drawPoint(float x, float y)          点を描画
 * drawPoint(float x, float y, float r) 半径を指定して点を描画
 * drawString(String text, float x, float y)                文字列を描画
 * drawStringCenter(String text, float cx, float cy)        中心位置を指定して文字列を描画
 * drawStringWithBack(String text, float x, float y, int textColor, int backColor)  背景色を設定して文字列を描画
 * drawStringCenterWithBack(String text, float cx, float cy, int textColor, int backColor)  背景色を指定して中心位置に文字列を描画
 * drawString(String text, float x, float y, TEXTALIGNMENT ta)  アライメントを指定して文字列を描画
 * drawStringWithBack(String text, float x, float y, TEXTALIGNMENT ta, int textColor, int backColor)    背景色とアライメントを設定して文字列を描画
 * drawPathOnText(String text, float xs, float ys, float xe, float ye, TEXTALIGNMENT ta)    drawPathOnText(String text, float xs, float ys, float xe, float ye, TEXTALIGNMENT ta)
 * getTextSize()                        文字サイズを取得する
 * measureText(String text)             文字列の長さを取得
 * getAscent()                          文字のベースラインより上の高さを取得
 * getDecent()                          文字のベースラインよりも下の高さを取得
 * getStringRect(float x, float y, String text)             文字列の領域を取得
 * getStringRect(String text)           文字列の領域を取得
 * getString(float left, float top, String text)            文字列の領域を取得
 *
 * == タートルグラフィック ==
 * moveTo(float x, float y)             線分の始点を設定
 * moveTo(PointF p)                     線分の始点を設定
 * moveTo(PointF p, float ang)          線分の始点と描画方向を設定
 * getCurrentPoint()                    線分の始点位置の取得
 * setLineAngle(float ang)              線分の描画方向の設定
 * getLineAngle()                       線分の秒化方向の取得
 * amoveTo(float length, float ang)     線分の長さと秒化方向(相対角度)を指定して始点を移動
 * lineTo(float dx, float dy)           相対座標で終点を指定して線分を描画
 * lineTo(PointF p)                     相対座標で終点を指定して線分を描画
 * plineTo(float x, float y)            絶対座標で終点を指定して線分を描画
 * alineTo(float length, float ang)     長さと相対角度を指定して線分を描画
 * anglrNormalize(double ang)           角度を-pi～piの間に正規化する
 *
 * distance(float sx, float sy, float ex, float ey)         2点間の距離を求める
 * distance(PointF sp, PointF ep)       2点間の距離を求める
 *
 *
 */
public class YDraw {
